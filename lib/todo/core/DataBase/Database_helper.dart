...
class DataBaseHelper {
  ...
  void _createDb(Database db, int newVersion) async {
    await db.execute(
        'CREATE TABLE $taskTable($colId INTEGER PRIMARY KEY AUTOINCREMENT, $colTitle TEXT, $colDone TEXT, $colNotes TEXT)');
  }
  ...
  Future<int> updateTask(Task task, String id) async {
    var db = await this.database;
    if (kDebugMode) {
      print('_alexTR_logging_ : updateTask : task : isDone : ${task.isDone}');
    }
